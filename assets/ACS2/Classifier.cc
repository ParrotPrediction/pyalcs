...
{
  initialize(new Condition(cl->C), new Action(cl->A), new Effect(cl->E), new PMark(), time, cl->q, cl->r, cl->i, cl->tav);
}
...
int Classifier::doesLink(Classifier *cl)
{
  ...
  cpl->reset();cpl2->reset();pcpl->reset();pcpl2->reset();
...
int Classifier::doesLink(char chr, int pos, CharPosList *cpl2, CharPosItem **cpi2, ProbCharPosList *pcpl2, ProbCharPosItem **pcpi2) 
{
...
{
  switch(X_TYPE){
  case 0: C->uniformCrossover(cl2->C);break;
  case 1: C->onePointCrossover(cl2->C, Perception::length);break;
  case 2: C->twoPointCrossover(cl2->C, Perception::length);break;
  default: cout<<"X_TYPE is not set correctly!!!"<<endl; return;
  }
  q = (q + cl2->q) / 2.;
  r = (r + cl2->r) / 2.;
  cl2->q = q;
 ...
 */
Classifier::Classifier(Condition *con, Action *act, Effect *eff, int time, double quality, double rewardPrediction, double imRePrediction)
{
 ...
 */
void Classifier::initialize(Condition *con, Action *act, Effect *eff, PMark *mark, int time, double qual, double rew, double imRew, double avApplTime)
{
  C=con;
  A=act;
  E=eff;
...
ostream& operator<<(ostream& out, Classifier *cl)
{
  out<<cl->C<<" "<<cl->A<<" "<<cl->E<<" ";
  out<<cl->M<<" ";
  out<<cl->q<<"\t"<<cl->r<<"\t"<<cl->i<<"\t"<<cl->exp<<" "<<cl->tga<<"\t"<<cl->talp<<" "<<cl->tav<<" "<<cl->num;
  return out;
}
