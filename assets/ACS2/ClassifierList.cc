...
void ClassifierList::checkList() {
    ...
    for (listp = list; listp != 0; listp = listp->next) {
        if (listp->cl->getCondition() == 0 || listp->cl->getAction() == 0 || listp->cl->getEffect() == 0 ||
            listp->cl->getPMark() == 0 || listp->cl->getEffect()->isEnhanced()) {
            cout << this;
...
void ClassifierList::chooseBestAppAvAction(Action *act) {
    ...
    for (PureClassifierList *listp = list; listp != 0; listp = listp->next) {
        delayValues[listp->cl->getAction()->getNr()] +=
                listp->cl->getApplicationAverage() * listp->cl->getQuality() * listp->cl->getNumerosity();
...
void ClassifierList::chooseLookaheadAction(Action *act, ClassifierList *pop, Perception *situation) {
    ...
    for (listp = list; listp != 0; listp = listp->next) {
        ...
        if (bestKnowClassifier[actNr] == 0 && max < listp->cl->getQuality() * listp->cl->getRewardPrediction()) {
            max = listp->cl->getQuality() * listp->cl->getRewardPrediction();
            maxCl = listp->cl;
        } else if (bestKnowClassifier[actNr] != 0 && max < (listp->cl->getQuality() * listp->cl->getRewardPrediction() +
                                                            bestKnowClassifier[actNr]->getQuality() * GAMMA *
                                                            bestActValues[actNr]) /
                                                           (1 + bestKnowClassifier[actNr]->getQuality() * GAMMA)) {
            max = (listp->cl->getQuality() * listp->cl->getRewardPrediction() +
                   bestKnowClassifier[actNr]->getQuality() * GAMMA * bestActValues[actNr]) /
                  (1 + bestKnowClassifier[actNr]->getQuality() * GAMMA);
            maxCl = listp->cl;
        }
    }
...
void ClassifierList::doOneStepMentalActing(int steps) {
    ...
    if (relList->size > 0) {
        ...
        for (i = 0; i < steps; i++) {
            ...
            for (j = 0, listp = relList->list;
                 j != chosen; ++j, listp = listp->next);//directly sets to chosen classifier
...
void ClassifierList::selectParents(Classifier **parent1, Classifier **parent2) {
    ...
    for (listp = list, qSum = 0; listp != 0; listp = listp->next)
        qSum += listp->cl->getQuality() * listp->cl->getQuality() * listp->cl->getQuality() *
                listp->cl->getNumerosity();
    ...
    for (listp = list, qCounter = 0; listp != 0; listp = listp->next) {
        qCounter += listp->cl->getQuality() * listp->cl->getQuality() * listp->cl->getQuality() *
                    listp->cl->getNumerosity();
        if (qCounter > qSel1) {
            if (qSel2 != -1) {
                *parent1 = listp->cl;
 ...
 */
void ClassifierList::freeBidisearchStuff(ClassifierList **al1, Perception **ap1, int size1, ClassifierList **al2,
                                         Perception **ap2, int size2, ClassifierList *cllist) {
    int i;
    for (i = 0; i < size1 - 1; i++) {
        delete al1[i];
        delete ap1[i];
 ...
 */
Action **
ClassifierList::searchOneBackwardStep(ClassifierList *relList, int *bs, int bSize, int bPoint, Perception **arrayPB,
                                      int fSize, Perception **arrayPF, ClassifierList **arrayListB,
                                      ClassifierList **arrayListF) {
        ...
        if (matchBW->size > 0) {
            ...
            for (PureClassifierList *listp = matchBW->list; listp != 0; listp = listp->next) {
                ...
                if (ant != 0) {//backwards anticipation was formable
                    if (doesContainState(arrayPB, *bs, ant) ==
                        -1) {//state not detected backward -> search in forward list
                        int fwseq = -1;
