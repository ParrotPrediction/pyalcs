...
class ProbabilityEnhancedAttribute(dict):
    ...
    @classmethod
    def merged_attributes(cls, attr1, attr2,
    ...
    def insert_attribute(self, other, q1, q2):
        assert isinstance(other, ProbabilityEnhancedAttribute)

        for symbol \
                in self.symbols_specified().union(other.symbols_specified()):
            self[symbol] = self.get(symbol, 0.0) * q1 + \
                other.get(symbol, 0.0) * q2
        self.adjust_probabilities()
