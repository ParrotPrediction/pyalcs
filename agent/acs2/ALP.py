...
class ALP:
    ...
    @staticmethod
    def apply(classifiers: list, action: int, time: int, action_set: list, perception: list, previous_perception: list):
        was_expected_case = 0
    ...
    def _update_application_average(cls: Classifier, time: int):
        ...
        cls.tga = time  # TGA? Should this be in ALP module? Maybe naming convention should be changed

    @staticmethod
    def _does_anticipate_correctly(cls: Classifier, perception: list, previous_perception: list) -> bool:
        """
        ...
        for i in range(const.CLASSIFIER_LENGTH):
            if cls.effect == const.CLASSIFIER_WILDCARD:
                ...
                if cls.effect[i] != perception[i] or previous_perception[i] == perception[i]:
                    return False
