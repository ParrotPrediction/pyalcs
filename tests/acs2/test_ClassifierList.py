...
class TestClassifierList:
    ...
    def test_should_select_parents1(self, cfg):
        ...
        # when
        p1, p2 = population.select_parents(randomfunc=(RandomMock([0.7, 0.1])))
    ...
    def test_exists_classifier(self, cfg):
        ...
        # C1 - OK
        c1 = Classifier(condition='0##0####', action=0, effect='1##1####', quality=0.7, cfg=cfg)

        # C2 - wrong action
        c2 = Classifier(condition='0##0####', action=1, effect='1##1####', quality=0.7, cfg=cfg)

        # C3 - wrong condition
        c3 = Classifier(condition='0##1####', action=0, effect='1##1####', quality=0.7, cfg=cfg)

        # C4 - wrong effect
        c4 = Classifier(condition='0##0####', action=0, effect='1##0####', quality=0.7, cfg=cfg)

        # C5 - wrong quality
        c5 = Classifier(condition='0##0####', action=0, effect='1##1####', quality=0.25, cfg=cfg)
        ...
        # when
        result0 = population.exists_classifier(previous_situation=prev_situation,
                                                 situation=situation, action=act, quality=q)

        population.append(c1)
        result1 = population.exists_classifier(previous_situation=prev_situation,
                                               situation=situation, action=act, quality=q)
